...
function CheckoutContent() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"M-Pesa" | "Card" | "COD">("M-Pesa");
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (paymentMethod === "M-Pesa" || paymentMethod === "Card") {
        ...
        const handler = window.PaystackPop.setup({
            ...
            metadata: {
                custom_fields: [
                    {
                        display_name: "Payment Selection",
                        variable_name: "payment_selection",
                        value: paymentMethod
                    }
                ]
            },
            callback: async (response: PaystackResponse) => {
                if (response.status === 'success' && supabase) {
                    await supabase.from('orders').update({
                        ...
                        note: `Paystack Ref: ${response.reference} | Method: ${paymentMethod}`
