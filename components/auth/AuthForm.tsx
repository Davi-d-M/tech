...
export default function AuthForm({ initialMode = 'signin' }: AuthFormProps) {
    ...
    const handleAuth = async (e: React.FormEvent) => {
        e.preventDefault();
        console.log("[AUTH] Triggering authentication protocol...");
        ...
        try {
            ...
            console.log("[AUTH] Sign in success. Reconciling identity signals...");

            // 🧠 Signal Intelligence: Non-blocking Identity Bridge
            setTimeout(async () => {
                if (!supabase) return;
                try {
                    ...
                    console.warn("[SIGNAL] Identity bridge background failure:", e);
                }
            }, 10);
            ...
            if (errorMsg.includes('email rate limit') || errorMsg.includes('rate limit')) {
                ...
                setMessage('Invalid email or password. Ensure your account is verified via email.');
            } else {
                setMessage(errorMsg);
            }
        } finally {
        ...
        <div className="max-w-md w-full mx-auto p-6 bg-white rounded-2xl border border-slate-100 shadow-sm">
            ...
            <form onSubmit={handleAuth} className="space-y-4">
                ...
                <button
                    ...
                    disabled={loading}
                    className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 hover:bg-slate-50 text-foreground font-black uppercase text-[10px] tracking-widest py-3.5 rounded-xl transition-all active:scale-95 disabled:opacity-50"
                >
            ...
            {message && (
                <p className="mt-4 text-sm text-center font-medium text-slate-600 bg-slate-50 p-2.5 rounded-xl border border-slate-100 animate-in fade-in">
                    {message}
                </p>
            )}
