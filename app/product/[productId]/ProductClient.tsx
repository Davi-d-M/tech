...
export default function ProductClient({
    ...
    product: ProductType;
    reviewStats: { count: number; rating: number };
    tutorials: Tutorial[];
}) {
  ...
  useEffect(() => {
    ...
    if (product) {
        ...
        views = views.filter((v: Record<string, unknown>) => v.id !== product.id);
        views.unshift({ id: product.id, name: product.name, image: product.image_url || product.image });
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 text-left">
      <JsonLd
        ...
        breadcrumbs={[
          { name: 'Home', item: '/' },
          { name: 'Shop', item: '/shop' },
          { name: product.category || 'Gadget', item: `/shop/category/${product.category}` },
      ...
      <ReviewSection productId={product.id} isLive={true} />
