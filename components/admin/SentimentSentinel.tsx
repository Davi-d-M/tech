...
export default function SentimentSentinel() {
    ...
    return (
        <Card className="p-8 rounded-[3rem] bg-rose-50 text-rose-600 border border-rose-100 shadow-sm animate-in zoom-in-95 duration-500 relative overflow-hidden group">
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                <div className="flex items-center gap-6 text-left">
                    <div className="h-16 w-16 rounded-[1.8rem] bg-white flex items-center justify-center border border-rose-100 shadow-sm">
                        <TrendingDown className="h-8 w-8 text-rose-500 animate-bounce" />
                    </div>
                    <div>
                        <h3 className="text-xl font-black uppercase tracking-tighter leading-none text-rose-900">Sentinel: Vibe Check</h3>
                        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-rose-400 mt-1">Brand Temperature: <span className={cn(temperature > 100 ? "text-rose-600" : "text-rose-400")}>{temperature.toFixed(1)}°F</span></p>
                        <p className="text-[10px] font-medium text-rose-500 italic mt-2">&quot;Negative customer sentiment detected in the extraction grid.&quot;</p>
                    </div>
                </div>

                <div className="flex flex-col gap-3 flex-1 w-full max-w-md">
                    {hotIssues.map((issue, i) => (
                        <div key={i} className="p-4 rounded-2xl bg-white border border-rose-100 flex justify-between items-center group/issue hover:bg-rose-50/50 transition-all shadow-sm">
                            <div className="flex items-center gap-3">
                                <AlertCircle size={14} className="text-rose-500" />
                                <p className="text-[10px] font-black uppercase truncate max-w-[200px] text-rose-900">{issue.type}: &quot;{issue.body}&quot;</p>
                            </div>
                ...
                <Link href="/admin/support/vip-relay">
                    <Button className="h-14 px-8 rounded-2xl bg-rose-600 text-white font-black uppercase text-[10px] tracking-widest shadow-xl shadow-rose-200 hover:bg-rose-700 active:scale-95 transition-all">
